...
M.opts = {
  ...
  current_line_blame_formatter =
    " >>> <author>, <author_time:%R> - <summary> <abbrev_sha>",
...
M.keys = {
  ...
  -- 自定义 commit 展示
  { "<leader>gc", function() _G.__gitsigns_show_commit() end, desc = "Show Commit" },
}
...
M.config = function(_, opts)
  ...
  local show_commit = async.async(function(win, open, bcache)
    ...
    api.nvim_buf_set_option(commit_buf, "foldmethod", "expr")
    api.nvim_buf_set_option(commit_buf, "foldexpr", "v:lua.GitFoldExpr(v:lnum)")
    api.nvim_buf_set_option(commit_buf, "foldtext", "v:lua.GitFoldText()")
    api.nvim_buf_set_option(commit_buf, "foldenable", true)
  ...
  --全局函数（给 keymap 调用）
  -------------------------------------------------------
  _G.__gitsigns_show_commit = function()
    ...
    -- 确保缓存和blame数据存在
    if not bcache then
      vim.notify("Gitsigns not attached", vim.log.levels.WARN)
      return
    end
